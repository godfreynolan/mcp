...
async def main() -> None:
    mcp_token = os.environ.get("RMOBILE_MCP_TOKEN", "")
    if not mcp_token:
        raise RuntimeError(
            "Set RMOBILE_MCP_TOKEN to either the read-only or admin token."
        )
    ...
    async with MCPServerStreamableHttp(
        ...
        agent = Agent(
            ...
            instructions=(
                ...
                "and to change a customer's email address when requested. "
