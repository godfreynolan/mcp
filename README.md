...
- Use the per-subdirectory `requirements.txt` files when working with that example.
- Tests are provided as `test_client.py` scripts that exercise the local servers; run them while the server is running.

## Questions / Next steps

- Want a consolidated requirements file and a single script to run all examples? I can add a top-level `requirements.txt` and simple orchestration script.
- Need a CONTRIBUTING.md or CI to run the example tests automatically? I can add a GitHub Actions workflow for that.

---

README generated by GitHub Copilot on request.